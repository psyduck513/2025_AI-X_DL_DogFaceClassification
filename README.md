...
강아지 이미지를 입력값으로 넣으면, 강아지의 얼굴 영역을 인식하고 기분상태를 판단하는 모델을 생성하고자 합니다.  
즉 **1) 강아지의 얼굴을 인식하는 모델**과 **2) 강아지의 감정을 분류하는 모델**, 총 2가지의 예측 모델을 예상 결과물로 두었습니다.  


# 2.Datasets
## 1) 강아지 얼굴 인식 모델 학습용 데이터

강아지 얼굴 인식 모델이 강아지 얼굴을 자동으로 인식하고 crop하도록 학습시키는데에는 Roboflow에 공개되어 있는 ‘Dog’ Dataset을 사용하였습니다.  
...
## 2) 강아지 감정 분류 모델 학습용 데이터

강아지 얼굴 사진을 ‘angry’, ‘happy’, ‘relaxed’, ‘sad’의 네 가지 감정으로 분류하기 위해, Roboflow에 공개된 ‘dog_emotions’ 데이터셋을 사용하였습니다.  
...
# 3.Methodology
## 1) 사용한 알고리즘(모델)
해당 프로젝트를 진행함에 있어 크게 두 가지 알고리즘(모델)을 사용하였습니다. 
강아지 얼굴 인식에는 YOLO v8 nano 모델을, 강아지 감정 분류에는 MobileNetV2 모델을 선정했습니다.

### YOLO v8
강아지 얼굴 인식 모델을 만들기 위해서는 YOLO 알고리즘을 활용했습니다.  

이중 YOLO v8은 Ultraylytics가 2023년에 공개한 실시간 객체 탐지 모델로 이미지를 처리하여 객체를 탐지하고 분류하는 모델입니다.    
기존 버전에 비해 성능과 속도 모두 향상되었고, 특히 작은 물체 탐지 성능이 우수하여 해당 프로젝트에 적합하다 판단했습니다.  
2025년 6월 기준, 버전 11까지 출시는 되었지만 YOLOv9~11는 대부분 연구자용으로 비전문가나 일반 프로젝트에는 부담이 큽니다. 
이에 비해 YOLO v8은 충분한 실사용 검증이 이루어져 가장 많이 사용되는 가장 안정적인 모델이기에, 본 프로젝트에 사용하기로 결정했습니다.

![image](https://github.com/user-attachments/assets/7d0eb5f6-2109-4a6b-b4f4-a241d39f0869)

YOLO v8 버전의 큰 특징으로는 Anchor Free Detection이 있습니다.  
기존에는 여러개의 예측될법한 Box들의 초기값을 설정(anchor) 후 이 값들을 통해 실제 검출되는 객체의 크기가 결정되는 방식으로 모델이 구동되었습니다.  
하지만 YOLO v8의 경우에는 미리 입력된 Anchor Box를 사용하지 않고, 객체의 center를 직접 예측하는 방법을 사용합니다.  

![image](https://github.com/user-attachments/assets/13b0fd3c-01c5-43d1-946c-dee58d9ce7ad)

YOLO v8에도 nano부터 extra large까지 속도와 정확성이 상이한 다양한 하위 모델이 존재합니다.  

정확성과 속도 요소는 서로 반비례에 있기에 둘 중 하나를 중점으로 모델링을 진행해야 했습니다.  

프로젝트 참여자의 PC 리소스로는 정확성에 초점을 둔 학습을 진행하기 어려웠으며,  
단 몇 초의 차이로 사고가 나고 안 나고가 결정되는 자율주행의 경우와 같이 속도가 정확성보다 중요한 상황이 충분히 존재하므로  
정확도보다는 속도에 중점을 두어 모델링을 진행하기로 결정하였습니다.  

따라서 YOLO v8의 nano, small, medium, large, extra large 모델 중 속도 상 가장 우수한 nano 모델을 활용했습니다.  
<img width="601" alt="image" src="https://github.com/user-attachments/assets/efa957ee-9d99-4e92-bbf5-897d188e99a4" />

### MobileNetV2
앞서 속도를 중점으로 모델링를 수행하기로 결정했기 때문에,  
강아지 감정 분류 모델링을 위해 수행 속도가 빠른 것으로 유명한 MobileNetV2를 사용하였습니다.  

MobileNetV2는 2018년 구글이 발표한 경량화 딥러닝 모델입니다.  
설계 특징으로는 linear transformation역할을 하는 linear bottleneck layer를 통해,  
차원은 줄이되 중요한 정보(manifold of interest)를 그대로 유지하여 네트워크 크기를 줄이면서도 정확도는 유지하는 모델입니다.  

![image](https://github.com/user-attachments/assets/7b6ff113-57fa-4a62-8fd9-f272a135e42e)
MobileNetV2는 고차원 이미지를 저차원에서의 다양한 특징으로 mapping하는데, 이 과정을 manifolds of interest를 구성한다고 합니다.  
이후, 이 manifolds of interest를 통해 저장된 정보들이 다시 layers를 거쳐 저차원 영역으로 전달되고, linear transformation을 통해 정보를 보존하게 됩니다.  

![image](https://github.com/user-attachments/assets/f46f1584-b6a8-4c61-b354-e9d491866e7c)

보존된 유용한 특징(feature)을 압축·확장하면서 효율적으로 표현하는 블록이 inverted residual block입니다.  
이를 숫자 벡터들로 정리, fully connected layer와 Softmax를 통해 특징들을 조합 후 클래스에 대한 확률로 추출합니다.  
...
YOLO 모델의 아키텍처를 설정하는 yaml 파일의 경우, 데이터셋 zip 파일에 내포되어 있던 yaml 파일을 그대로 사용했습니다.  
...
불러온 yolov8 nano 모델의 경우 COCO 데이터셋을 통해 사전 학습된 모델입니다. 코드를 통한 학습 과정에서는 출력층에 대한 학습만 진행합니다.  
...
base_model의 trainable 속성은 False로 설정해 사전 학습된 가중치에 대한 신규 학습 과정 없이, 그대로 사용하도록 합니다.  
...
옵티마이저의 경우 Adam으로 설정했습니다. 최근 경로 변화량에 적응적으로 학습하며 진행하던 최적화 속도에 관성을 부여하는 옵티마이저로, 가장 대중적으로 사용되기에 선정했습니다.  
손실 함수는 다중 분류 문제이기에 크로스엔트로피 함수로 설정했습니다.  
또한 성능 지표의 경우 다중 분류 문제에 대해 직관적으로 적용하기 쉬운 정분류율(accuracy)를 사용했습니다.  
...
출력층에 Dropout을 추가하여 과대적합을 방지해보고자 했으나 유의미한 성능 향상은 없었습니다.  
해당 과정은 4. Evaluation & Anlysis에서 간단히 설명드리겠습니다.  
...
성능 지표로는 Precision(정밀도), Recall(재현율), mAP를 선정했습니다.  
...
그렇다면 이제 어떠한 경우에 박스를 적절히 그렸다 판단할지에 대한 기준이 필요합니다.  
...
각 에포크에 따라 검증 데이터 성능 지표를 검토한 결과는 하기 시각화와 같습니다.  
...
오히려 에포크별 성능 지표가 조금 더 불규칙적으로 변했습니다.  
...
본 프로젝트를 통해 강아지 얼굴을 인식하는 YOLO v8 모델과 강아지 감정을 분류하는 MobileNetV2 모델을 제작해보았습니다.  
YOLO v8 모델은 IoU 0.5 기준으로 정답여부를 분류할 때 Precision, Recall, mAP가 모두 0.95 이상인 준수한 성능을 보이나,  
MobileNetV2 모델은 0.7이하의 Accuracy 성능에 그쳤습니다.  

모델을 적용해 Dog 데이터셋 중 검증 데이터에 박스 영역 및 감정 분류값을 직접 기재해보았고,  
해당 이미지 데이터는 본 깃헙 리포지토리에 업로드하였습니다.  
코랩을 통해 작성한 전체 코드의 파이썬 노트북 파일 또한 업로드해두었습니다.  

본 프로젝트를 통해 딥러닝 모델의 기본적인 원리와 CNN과 관련된 매커니즘, 하이퍼파라미터값에 대해 배울 수 있었습니다.
또한 YOLO v8, MobileNetV2와 같은 비교적 최신 모델을 직접 사용해볼 수 있었고,  
현재 실제로 딥러닝이 응용되는 상황에 대해 개괄적으로나마 이해할 수 있었습니다.  

다만 MobileNetV2 모델의 경우 충분한 성능을 확보하지 못했기 때문에  
배포가 가능한 수준의 모델을 제작하려면 앞으로 모델 원리에 대한 심층적인 이해와 디테일한 튜닝 능력이 필요할 것이라 사료됩니다.  
